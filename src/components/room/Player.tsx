...
function getUrlType(url: string): UrlType {
    ...
    try {
        ...
        if (pathname.endsWith('.mp4') || pathname.endsWith('.webm') || pathname.endsWith('.ogv') || pathname.endsWith('.m3u8')) return 'direct';
    } catch (e) {
      if (url.match(/^[a-zA-Z0-9_-]{11}$/)) return 'youtube';
    }
    return 'iframe';
}
...
function getYouTubeVideoId(url: string): string | null {
  ...
  try {
    ...
    if (hostname === 'www.youtube.com' || hostname === 'youtube.com') {
      ...
      if (urlObj.pathname.startsWith('/embed/')) return urlObj.pathname.split('/embed/')[1].split('?')[0];
    }
  } catch (e) {}
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
        ...
        switch (e.key.toLowerCase()) {
            ...
            case 'f': e.preventDefault(); if (containerRef.current?.requestFullscreen) { if (document.fullscreenElement) document.exitFullscreen(); else containerRef.current.requestFullscreen(); } break;
            case 'm': e.preventDefault(); handleVolumeChange([volume === 0 ? 0.8 : 0]); break;
        }
    };
      ...
      <div className="absolute inset-0 w-full h-full pointer-events-none">
        {urlType === 'youtube' && videoId && (
            <YouTube
                ...
                opts={{ height: '100%', width: '100%', playerVars: { autoplay: playerState?.isPlaying ? 1 : 0, start: Math.floor(Math.max(0, (playerState?.seekTime ?? 0) + (playerState?.isPlaying ? (getServerTimeNow() - (playerState?.timestamp || getServerTimeNow())) / 1000 : 0))), controls: 0, rel: 0, showinfo: 0, modestbranding: 1, iv_load_policy: 3, disablekb: 1, playsinline: 1, ...(quality !== 'auto' && {vq: quality}) }}}
                onReady={onYtReady} onStateChange={onYtStateChange} onEnd={() => canControl && onVideoEnded()} onError={() => canControl && onVideoEnded()} className="w-full h-full"
            />
        )}
        {urlType === 'direct' && (
            <video
                ...
                className="w-full h-full object-contain" onLoadedData={onHtmlReady} onPlay={onHtmlStateChange} onPause={onHtmlStateChange} onEnded={() => canControl && onVideoEnded()} playsInline autoPlay={playerState?.isPlaying}
            />
        )}
        {urlType === 'iframe' && <iframe src={videoUrl} title="Shared Content" className="w-full h-full border-0" allow="autoplay; encrypted-media; picture-in-picture; fullscreen" allowFullScreen></iframe>}
        {urlType === 'empty' && (
